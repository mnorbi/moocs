...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    if (doLogZ)
        %%% YOUR CODE HERE:
		logZ = log(sum(FactorProducts(unnormalizedMessages(:)).val));
    else
        logZ = 0;
    end
...
function Fnew = FactorProducts(F)
	Fnew = F;
	if length(F) > 1
		Fnew = F(1);
		for i = 2:length(F)
			Fnew = FactorProduct(Fnew, F(i));
		end
	end
end

function Fnew = FactorSums(F)
	Fnew = F;
	if length(F) > 1
		Fnew = F(1);
		for i = 2:length(F)
			Fnew = FactorSum(Fnew, F(i));
		end
	end
end
