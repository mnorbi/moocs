...
% all = 1:numPeople;
% processed = [];
% remaining = setdiff(all, processed);
% i = 0;
% while(remaining),
% 	i = mod(i+1,length(remaining))+1;
% 	idx = remaining[i];
% 	parents = pedigree.parents(idx,:);
% 	if (all(parents == [0,0])),
% 		factorList(idx) = genotypeGivenAlleleFreqsFactor(alleleFreqs, idx);
% 		factorList(idx+numPeople) = phenotypeGivenGenotypeFactor(alphaList, idx, numPeople+idx);
% 		processed = [processed, idx];
% 	elseif (all(ismember(parents, processed))),
