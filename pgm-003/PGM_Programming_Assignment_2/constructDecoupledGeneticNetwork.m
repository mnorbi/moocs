...
for i=1:numPeople,
 	...
 	if (all(parents == [0,0])),
		...
		factorList(numPeople+i) = childCopyGivenFreqsFactor(alleleFreqs, numPeople+i);
 		...
 		factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, parents(1,1), numPeople+parents(1,1));
 		factorList(numPeople+i) = childCopyGivenParentalsFactor(numAlleles, numPeople+i, parents(1, 2), numPeople+parents(1, 2));
	end;
