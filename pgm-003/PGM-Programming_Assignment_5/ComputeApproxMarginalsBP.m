...
function M = ComputeApproxMarginalsBP(F,E)
    ...
    for i = 1:length(N),
		...
		targetVar = N(i);
		clusterWithTargetVar = P.clusterList(FindPotentialWithVariable(P, targetVar));
		M(i) = FactorMarginalization(clusterWithTargetVar, setdiff(clusterWithTargetVar.var, targetVar));
