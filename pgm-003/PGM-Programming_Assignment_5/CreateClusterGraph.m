...
P.clusterList = F;
singletonClusterIdxs = find(arrayfun(@(struct) length(struct.var) == 1, P.clusterList));
%singletonClusterVars = arrayfun(@(struct) struct.var, P.clusterList(singletonClusterIdxs))
P.edges = zeros(length(P.clusterList));
for i = 1:length(singletonClusterIdxs),
	clusterI = P.clusterList(i);
	P.edges(i,:) = arrayfun(@(struct) length(struct.var) > 1 && intersect(struct.var, clusterI.var), P.clusterList);
	P.edges(:,i) = P.edges(i,:)';
end;
